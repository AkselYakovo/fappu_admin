  ...
  constructor(targetNode) {
    ...
    targetNode.addEventListener("mousedown", (e) => {
      ...
      targetNode.initialCoords = {
        X: getTranslateX(targetNode) ? getTranslateX(targetNode) : "0",
        Y: getTranslateY(targetNode) ? getTranslateY(targetNode) : "0",
      }
      ...
      document.onmousemove = function (event) {
        ...
        let finalPositionX =
          targetNode.initialCoords.X -
          (initialMouseCoords.X - currentMouseCoords.X)
        let finalPositionY =
          targetNode.initialCoords.Y -
          (initialMouseCoords.Y - currentMouseCoords.Y)
        if (!this.isAdjusting) {
      ...
      document.onmouseup = function () {
        ...
        if (targetNode.finalCoords.topLeft.X > parentCoords.topLeft.X) {
          ...
          let newCoord =
            targetNode.finalCoords.topLeft["X"] - parentCoords.topLeft["X"]
        ...
        // RIGHT EDGE AUTO POSITIONING...
          ...
          let actualCoord = getTranslateX(targetNode)
          let newCoord =
            parentCoords.topRight["X"] - targetNode.finalCoords.topRight["X"]
          finalCoord.X = actualCoord + newCoord
        }
        ...
        if (targetNode.finalCoords.topLeft.Y > parentCoords.topLeft.Y) {
          ...
          let newCoord =
            targetNode.finalCoords.topRight["Y"] - parentCoords.topRight["Y"]
          finalCoord.Y = actualCoord - newCoord
        }
        // BOTTOM EDGE AUTO POSITIONING...
        else if (
          targetNode.finalCoords.bottomRight.Y < parentCoords.bottomRight.Y
        ) {
          // console.log('Excess N\' Bottom!')
          this.isAdjusting = true
          let actualCoord = getTranslateY(targetNode)
          let newCoord =
            parentCoords.bottomRight["Y"] -
            targetNode.finalCoords.bottomRight["Y"]
          finalCoord.Y = actualCoord + newCoord
        }
